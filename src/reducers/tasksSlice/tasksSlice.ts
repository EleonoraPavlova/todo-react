...
export const updateTaskTC = createAsyncThunk(
  'tasks/updateTask',
  async (
    param: { todoListId: string; id: string; apiModal: UpdateTaskModelForReducerFn },
    { dispatch, rejectWithValue, getState }
  ) => {
    const state = getState() as AppRootState
    const task = state.tasks[param.todoListId].find((t: TaskTypeApi) => t.id === param.id) //вытянула rootReducer с тасками и нашла нужную
    if (task) {
...
export const changeTaskStatusTC =
  ...
  async (dispatch, getState: () => AppRootState) => {
    dispatch(setAppStatusAC({ status: 'loading' }))
    const task = getState().tasks[todoListId].find((t: TaskTypeApi) => t.id === id) //вытянула rootReducer с тасками и нашла нужную
    if (task) {
